...
function searchSongs(){
  ...
  fetch(baseQuery+URLencoded+apiKey) // ajax request
...
function saveToSonglist(trackID){
  ...
  let user = auth.currentUser.uid
   ...
   db.collection('songs').doc(user).get().then((docSnapshot) =>{ // checks to see if the user already has a document created
      ...
      else{db.collection('songs').doc(user).set({ // if no document create new document and add the content
          song: songArray
          } ,{ merge: true })}
  }
  )
  ...
  {
      existingData = data.data().song // returns an array - the users existing data
