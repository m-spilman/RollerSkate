...
document.addEventListener('DOMContentLoaded', function () {
  auth.onAuthStateChanged(function (user) {
    if (user) { db.collection('songs').doc(auth.currentUser.uid).get().then(renderMe) } // if user is logged in, get their data and call renderMe
    else { renderNoUser() } // if user is not logged in call renderNoUser
  })
})
// renders the data retreived from the users document
function renderMe (data) {
  let songsArray = data.data().song
  let songListHTML = songsArray.map(function (data) {
